...
// Presupuesto = precio (recalculado post-corte) del MEJOR jugador
// del equipo que no pasó el corte.
export function replacementBudget(
  picks: Pick[],
  players: Player[]
): { needsReplacement: boolean; budget: number; missedCutCount: number } {
  const missed = playersMissedCut(picks, players);
  const needsReplacement = missed.length >= 3;
  if (!needsReplacement) {
    return { needsReplacement: false, budget: 0, missedCutCount: missed.length };
  }
  // "Mejor jugador que no pasó el corte" = precio actual más alto
  const budget = Math.max(...missed.map(p => p.currentPrice));
  return { needsReplacement: true, budget, missedCutCount: missed.length };
}
