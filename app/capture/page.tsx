...
export default function CapturePage() {
  ...
  async function loadTournament() {
    ...
    if (t) {
      ...
      // Solo golfistas que fueron elegidos por al menos un participante
      const { data: pickData } = await supabase.from('picks').select('player_id').eq('tournament_id', t.id);
      const pickedIds = Array.from(new Set((pickData || []).map(p => p.player_id)));
  ...
  async function saveAll() {
    ...
    const toUpsert = Object.entries(holes)
    ...
    const holesToClear = Object.entries(holes)
      .filter(([, result]) => result === '')
      .map(([hole]) => Number(hole));

    if (holesToClear.length > 0) {
      await supabase.from('hole_results').delete()
        .eq('tournament_id', tournament.id).eq('player_id', selectedPlayer).eq('round', round)
        .in('hole', holesToClear);
    }

    const { error } = toUpsert.length > 0
      ? await supabase.from('hole_results').upsert(toUpsert, { onConflict: 'player_id,round,hole' })
      : { error: null };
