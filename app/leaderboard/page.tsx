  ...
  async function load() {
    ...
    const result: TeamStanding[] = Object.entries(byParticipant).map(([participantId, picks]) => {
      ...
      const roster = picks.map(pk => {
        ...
        const playerTotal = round1 + round2 + round3 + round4 + cutBonus + finBonus;
        return {
      ...
      const roundTotals = {
        ...
        cutBonus: roster.reduce((sum, r) => sum + r.cutBonus, 0),
        finishBonus: roster.reduce((sum, r) => sum + r.finishBonus, 0),
      };
      const name = participants.find(p => p.id === participantId)?.name || '?';
      return { participantId, participantName: name, total, roundTotals, roster };
