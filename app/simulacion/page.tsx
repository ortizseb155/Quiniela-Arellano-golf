...
import {
  HoleResult, POINT_VALUES, teamTotalPoints, playerRoundHolePoints,
  finishBonus, BONUS, Player, HoleResultRow, Pick,
} from '@/lib/scoring';
...
export default function SimulacionPage() {
  ...
  async function load() {
    ...
    const [{ data: playersData }, { data: officialData }, { data: mineData }, { data: picksData }, { data: participantsData }] = await Promise.all([
  ...
  function computeStandings() {
    ...
    const result: TeamStanding[] = Object.entries(allPicks).map(([participantId, picks]) => {
      const total = teamTotalPoints(picks, players, merged, roundsPlayed);
      const roster = picks.map(pk => {
        ...
        const finBonus = player ? finishBonus(player) : 0;
        return {
  ...
  async function saveSimulation() {
    ...
    const rows = Object.entries(editHoles)
    ...
    const { error } = await supabase.from('simulated_hole_results').upsert(rows, { onConflict: 'participant_id,player_id,round,hole' });
